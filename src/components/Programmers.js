...
export default function Programmers() {
  ...
  let [programmers, setProgrammer] = useState(listOfAwesome);
  let [currentCoder, setFeaturedProgrammer] = useState(null);
  const getNameOfFeatured = () => {
    ...
    return programmers.filter(dev => dev.id === currentCoder)[0].name;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: currentCoder > 0 ? 'gold' : 'royalblue'
  };
